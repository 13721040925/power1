	...
	<div style="width: 70%;margin: 0 auto;">
		<form action="pow/dangerInfos" method="post" style="margin-top: 130px;text-align: center;">
			...
			<table class="table table-bordered" >
				...
				<tr>
					  <td class="" style="line-height:35px;font-size: 17px; ">${warn.ip }</td> 
					  <td class="" style="line-height:35px;font-size: 17px; ">${warn.warntime }</td>
					  <td class="" style="line-height:35px ;font-size: 17px;">${warn.group_name }</td>
					  <c:if test="${warn.powertemplower==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">温度过低</td>
					  </c:if> 
					  <c:if test="${warn.powertemphigher==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">温度过高</td>
					  </c:if>
					  <c:if test="${warn.indivhighers==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">单体电池过压</td>
					  </c:if>
					  <c:if test="${warn.indivlowers==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">单体电池欠压</td>
					  </c:if>
					  <c:if test="${warn.groupvhigher==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">电池过压</td>
					  </c:if>
					  <c:if test="${warn.groupvlower==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">电池欠压</td>
					  </c:if>
					  <c:if test="${warn.powerahigher==1}">
					  	<td class="" style="line-height:35px ;font-size: 17px;">电池过流</td>
					  </c:if>
					  <c:if test="${warn.bmu==1}">
